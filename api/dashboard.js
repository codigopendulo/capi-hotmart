...
// Separar Hotmart vs Shopify
let hotmartRev=0, hotmartFees=0, hotmartSales=0;
let shopifyRev=0, shopifyFees=0, shopifySales=0;

const byProd={};
for(const s of allSales){
  const p=s.p||"Desconocido";
  if(!byProd[p])byProd[p]={rev:0,rev_main:0,rev_bump:0,n:0,n_main:0,bumps:0,fees:0};
  byProd[p].rev+=s.v;byProd[p].n++;
  if(s.src==="shopify"){
    byProd[p].fees+=s.v*MP_FEE;
    shopifyRev+=s.v;shopifyFees+=s.v*MP_FEE;shopifySales++;
  } else {
    byProd[p].fees+=HOTMART_FEE;
    hotmartRev+=s.v;hotmartFees+=HOTMART_FEE;hotmartSales++;
  }
...
for(const[p,d]of Object.entries(byProd)){
  ...
  const feesPerSale=d.n_main>0?d.fees/d.n_main:0;
...
const totalFbSpend=adsets.reduce((a,s)=>a+s.spend,0);
const hotmartNeto=hotmartRev-hotmartFees;
const shopifyNeto=shopifyRev-shopifyFees;
const totalIngresos=hotmartRev+shopifyRev;
const totalFees=hotmartFees+shopifyFees;
const ganancia=totalIngresos-totalFees-totalFbSpend;
...
<div class="label">GANANCIA NETA EN TIEMPO REAL</div>
<div class="amount ${ganancia>=0?'green':'red'}">${$(ganancia)}</div>
</div>

<div class="totals">
<div class="tc"><div class="l">Hotmart (INT)</div><div class="v gold">${$(hotmartRev)}</div><div class="detail">Neto: ${$(hotmartNeto)} (${hotmartSales} ventas, -$${HOTMART_FEE}/venta)</div></div>
<div class="tc"><div class="l">Shopify (AR)</div><div class="v gold">${$(shopifyRev)}</div><div class="detail">Neto: ${$(shopifyNeto)} (${shopifySales} ventas, -${(MP_FEE*100).toFixed(0)}% MP)</div></div>
<div class="tc"><div class="l">Gasto Facebook</div><div class="v red">${$(totalFbSpend)}</div><div class="detail">${adsets.length} conjuntos activos</div></div>
<div class="tc"><div class="l">Ventas Totales</div><div class="v">${hotmartSales+shopifySales}</div><div class="detail">${T.bumps} order bumps</div></div>
</div>

<h2>RENTABILIDAD POR PRODUCTO</h2>
<div class="info-box"><strong>Datos reales de Hotmart y Shopify.</strong> CPA Maximo = lo maximo que podes pagar por venta sin perder plata.</div>
<table><thead><tr>
...
<div class="note">Hotmart: -$${HOTMART_FEE}/transaccion | Shopify: -${(MP_FEE*100).toFixed(0)}% MercadoPago | TC: 1 USD = 1450 ARS | Facebook API cache 5min | ${new Date(Date.now()-3*60*60*1000).toISOString().slice(0,19)} AR</div>
</div></body></html>`;
