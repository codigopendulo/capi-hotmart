...
// Calcular métricas reales por producto
const prodMetrics={};
for(const[p,d]of Object.entries(byProd)){
  const bumpRate=d.n_main>0?d.bumps/d.n_main:0;
  const avgBumpsPerSale=bumpRate;
  const aovGross=d.n_main>0?d.rev/d.n_main:0;
  const feesPerSale=HOTMART_FEE*(1+avgBumpsPerSale);
  const aovNeto=aovGross-feesPerSale;
  const cpaBE=aovNeto>0?aovNeto:0;
  prodMetrics[p]={bumpRate,avgBumpsPerSale,aovGross,aovNeto,cpaBE,mult:d.rev_main>0?d.rev/d.rev_main:1};
}
...
if(spend>0&&purchases>0){
  if(cpaBE>0)status=cpa<=cpaBE*0.6?"ESCALAR":cpa<=cpaBE*0.85?"RENTABLE":cpa<=cpaBE?"BREAKEVEN":"MATAR";
  else status=roasReal>=3?"ESCALAR":roasReal>=2?"RENTABLE":roasReal>=1.5?"BREAKEVEN":"MATAR";
...
adsetHtml+=`<tr class="camp-row"><td colspan="9"><strong>${camp}</strong> <span class="dim">${$(cs)} spend | ${cp} ventas${campCpaBE>0?' | CPA breakeven: '+$(campCpaBE):''}</span></td></tr>`;
for(const a of sets){
...
<h1>MUSCULO LAB</h1>
<div class="sub">Dashboard de Rentabilidad — ${dateFrom} a ${dateTo}</div>
<div class="filters"><a href="?days=1" ${days===1?'class="active"':''}>Hoy</a><a href="?days=3" ${days===3?'class="active"':''}>3d</a><a href="?days=7" ${days===7?'class="active"':''}>7d</a><a href="?days=14" ${days===14?'class="active"':''}>14d</a><a href="?days=30" ${days===30?'class="active"':''}>30d</a></div>

<div class="totals">
<div class="tc"><div class="l">Revenue Hotmart</div><div class="v gold">${$(T.rev)}</div></div>
<div class="tc"><div class="l">Spend Facebook</div><div class="v">${$(T.spend)}</div></div>
<div class="tc"><div class="l">Profit Neto</div><div class="v ${T.profit>=0?'green':'red'}">${$(T.profit)}</div></div>
...
<h2>METRICAS POR PRODUCTO</h2>
<div class="info-box"><strong>Estas metricas se calculan con ventas reales de Hotmart.</strong> A medida que entren ventas, los numeros se actualizan solos. CPA Breakeven = AOV neto (despues de fees) — es el maximo que podes pagar por venta para no perder plata.</div>
<table><thead><tr>
<th>Producto</th><th style="text-align:right">Revenue</th><th style="text-align:right">Spend</th><th style="text-align:right">Profit</th>
<th style="text-align:center">Ventas</th><th style="text-align:center">Bumps</th><th style="text-align:center">Bump Rate</th>
<th style="text-align:right">AOV Bruto</th><th style="text-align:right">AOV Neto</th><th style="text-align:right">CPA BE</th><th style="text-align:center">Estado</th>
...
<h2>DECISION POR AD SET</h2>
<div class="info-box"><strong>CPA vs CPA Breakeven:</strong> Si tu CPA es menor al breakeven, ganas plata. <span class="green">Verde = CPA menor al 60% del BE (escalar)</span> | <span class="gold">Amarillo = entre 60-100% (rentable)</span> | <span class="red">Rojo = arriba del BE (perdes plata)</span></div>
<table><thead><tr>
<th>Ad Set</th><th style="text-align:right">Spend</th><th style="text-align:center">Ventas</th><th style="text-align:right">CPA</th><th style="text-align:right">CPA BE</th>
<th style="text-align:right">Rev FB</th><th style="text-align:right">Rev Real</th><th style="text-align:center">ROAS Real</th><th style="text-align:center">Estado</th>
</tr></thead><tbody>${adsetHtml||'<tr><td colspan="9" class="center dim" style="padding:20px">No hay datos de ad sets</td></tr>'}</tbody></table>

<div class="legend"><span>ESCALAR: CPA menor a 60% del breakeven</span><span>RENTABLE: CPA entre 60-85% del breakeven</span><span>BREAKEVEN: CPA entre 85-100%</span><span>MATAR: CPA arriba del breakeven</span></div>
<div class="note">Revenue: Hotmart webhooks | Spend: Facebook API | Fee Hotmart: $${HOTMART_FEE}/transaccion | ${new Date().toISOString().slice(0,19)} UTC</div>
</div></body></html>`;
